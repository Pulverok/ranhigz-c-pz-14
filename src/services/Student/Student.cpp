...
namespace services {
    void services::Student::PrintStudents(uint32_t count) {
        for (int i = 0; i < count; i++) {
            entities::Student student = this->fakeData.GenerateStudent();

            std::cout << "Name: " << student.GetPerson().GetFio().GetFullName() << std::endl;
            std::cout << "Age: " << (unsigned) student.GetPerson().GetAdditionalInfo().GetAge() << std::endl;
            std::cout << "Group: " << student.GetGroup() << std::endl;
            std::cout << "Course: " << (unsigned) student.GetCourse() << std::endl;
